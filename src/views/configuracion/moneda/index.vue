<template>
    ...
    <ion-content color="light">
      ...
      <ion-list :inset="true">
        <ion-item>
          ...
          <ion-label horizontal="center"><strong>Nombre</strong></ion-label>
          <ion-label horizontal="center"><strong>Abreviatura</strong></ion-label>
          <ion-label horizontal="center"><strong>Símbolo</strong></ion-label>
        ...
        <ion-item v-for="(fila, index) in data" :key="index">
          ...
          <ion-label><div style="text-align: center;">{{ fila.abreviatura }}</div></ion-label>
          <ion-label><div style="text-align: center;">{{ fila.simbolo }}</div></ion-label>
  ...
  <script setup>
    ...
    let listar = (async()=>{
      ...
      try {
        const res = await API_MONEDAS.get(param);
        data.value=res;
      } catch (error) {
    ...
    const alertButtons = [
      ...
      },
      {
        ...
        handler: async () => {
          try {
            await API_MONEDAS.destroy(idDelete.value);
  ...
  const presentAlert = async (id) => {
    const alert = await alertController.create({
      header: '',
      subHeader: '',
      message: 'Esta seguro de eliminar la moneda.',
